...
export class MembersController {
  ...
  async importFromExcel(
    ...
    for (const row of rows) {
      try {
        const genderVal = String(row['gender'] ?? row['gioi_tinh'] ?? row['Giới tính'] ?? '').toLowerCase();
        const levelVal = String(row['level'] ?? row['cap'] ?? row['Cấp'] ?? '').toLowerCase();

        const rawOrgUnit = row['organizationUnitId'] ?? row['don_vi_id'] ?? row['Đơn vị ID'];
        ...
        const dto: CreateMemberDto = {
          ...
          baptismName: row['baptismName'] != null ? String(row['baptismName']) :
            row['ten_thanh'] != null ? String(row['ten_thanh']) :
            row['Tên thánh'] != null ? String(row['Tên thánh']) : undefined,
          phone: row['phone'] != null ? String(row['phone']) :
            row['so_dien_thoai'] != null ? String(row['so_dien_thoai']) :
            row['Số điện thoại'] != null ? String(row['Số điện thoại']) : undefined,
          ...
          level: ['cap_1', '1', 'cấp 1', 'cap1'].includes(levelVal)
            ...
            : ['cap_2', '2', 'cấp 2', 'cap2'].includes(levelVal)
              ...
              : ['cap_3', '3', 'cấp 3', 'cap3'].includes(levelVal)
                ? MemberLevel.CAP_3
                : undefined,
          organizationUnitId: rawOrgUnit != null ? Number(rawOrgUnit) : undefined,
          notes: row['notes'] != null ? String(row['notes']) :
            row['ghi_chu'] != null ? String(row['ghi_chu']) :
            row['Ghi chú'] != null ? String(row['Ghi chú']) : undefined,
        };
